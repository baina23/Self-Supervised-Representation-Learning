...
class Algorithm():
    ...
    def solve(self, data_loader_train, data_loader_test, exp_name='', maximum_epochs=None):
        self.max_num_epochs = self.opt['max_num_epochs']
        if maximum_epochs is not None:
            self.max_num_epochs = maximum_epochs
        ...
        for self.curr_epoch in range(start_epoch, self.max_num_epochs):
            ...
            if data_loader_test is not None:
                ...
                #print(eval_stats['prec1'])
                #assert False
                #self.curr_epoch = 99
                if (self.curr_epoch+1) % 100 == 0:
                    print("------------ Saving evaluation results---------------")
                    print("epoch: {}".format(self.curr_epoch+1))
                    evaluate_stats_list.append(eval_stats['prec1'])
        np.savetxt(exp_name + ".csv", evaluate_stats_list, delimiter=",")            
    ...
    def run_train_epoch(self, data_loader, epoch):
        ...
        self.bnumber = int(data_loader.__len__())
...
#                 print(int(epoch+1))
#                 print(int(idx+1))
#                 print(int(data_loader.__len__()))
#                 print(train_stats.average())
                self.logger.info('==> Iteration [%3d][%4d / %4d]: %s' % (int(epoch+1), int(idx+1), int(data_loader.__len__()), train_stats.average()))
    ...
    def evaluate(self, dloader):
        ...
        self.logger.info('==> Dataset: %s [%d images]' % (dloader.dataset.name, int(dloader.__len__())))
        for key, network in self.networks.items():
            network.eval()

        eval_stats = utils.DAverageMeter()
        self.bnumber = int(dloader.__len__())
